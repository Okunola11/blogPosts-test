...
Begin by adding `djangorestframework` and `djangorestframework-simplejwt` to your **requirements.txt** file if you have:
...
In the api app, navigate to the _urls.py_ file, add the following
...
To require authentication globally, navigate to your `settings.py` file. If the **REST_FRAMEWORK** dictionary does not exist, create it. Then, add the following configuration:
